...
if ($conn->connect_error) {
  ...
  $typing = $_POST['typing'];
  if($choose=="studentID"){
  $query = "select O.orderid, O.userid,U.phone as uphone, O.tutorid as tutorid,U.name as username,O.location ,O.Date,time.timeslot,O.state,O.course,O.question, T.name from orders O, tutor T , users U, time where T.tutorid=O.tutorid and O.userid=U.userid and time.timeid = O.time and U.userid='$typing' order by O.Date desc, time.timeslot asc";}
  else if ($choose=="studentname"){
  $query = "select O.orderid, O.userid,U.phone as uphone, O.tutorid as tutorid,U.name as username,O.location ,O.Date,time.timeslot,O.state,O.course,O.question, T.name from orders O, tutor T , users U, time where T.tutorid=O.tutorid and O.userid=U.userid and time.timeid = O.time and U.name='$typing' order by O.Date desc, time.timeslot asc";
  }
